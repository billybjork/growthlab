...
**Do not add inline styles manually.** All styling should be handled via CSS classes in `public/css/styles.css`.

The only exception is **images and videos**, where inline styles are automatically generated by the WYSIWYG editor when you:
- Resize media using drag handles
- Change alignment using the alignment toolbar

These auto-generated styles store the specific dimensions and alignment for each piece of media. When writing content manually or via LLM, never add inline styles - use CSS classes instead.
